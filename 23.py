...
class OptimizerUI(GalaxyBackground):
    def __init__(self):
        ...
        subtitle = QLabel("Low-key, powerful, safe, universal optimization")
        ...
        self.cleaned_card.setFixedSize(180, 82)
        self.optimized_card.setFixedSize(180, 82)
        ...
        self.progress.setFixedWidth(620)
        ...
        self.safety_note = QLabel("Safe mode + restore point = protected optimization")
        self.safety_note.setFont(QFont("Segoe UI", 9))
        self.safety_note.setAlignment(Qt.AlignmentFlag.AlignCenter)

        # Settings panel (hidden until gear icon click)
        ...
        self.dark_mode_checkbox.toggled.connect(self.on_mode_changed)
        ...
        settings_layout.addWidget(self.dialog_hint)
    ...
    def apply_theme(self):
        if self.is_dark_mode:
            ...
            panel_border = "#334155"
            settings_btn_bg = "rgba(30, 41, 59, 0.85)"
            settings_btn_fg = "#e2e8f0"
        else:
            text_primary = "#0f172a"
            text_secondary = "#334155"
            text_muted = "#64748b"
            note_color = "#854d0e"
            panel_bg = "rgba(241, 245, 249, 0.9)"
            panel_border = "#cbd5e1"
            settings_btn_bg = "rgba(226, 232, 240, 0.95)"
            settings_btn_fg = "#0f172a"

        self.setStyleSheet(f"QWidget {{ background: transparent; }}")
        self.status.setStyleSheet(f"color: {text_primary};")
        self.substatus.setStyleSheet(f"color: {text_secondary};")
        self.safety_note.setStyleSheet(f"color: {note_color};")
        self.dialog_hint.setStyleSheet(f"color: {text_muted};")
        for box in (self.visual_fx_checkbox, self.show_completion_checkbox, self.dark_mode_checkbox):
            box.setStyleSheet(f"QCheckBox {{ color: {text_secondary}; font: 10pt 'Segoe UI'; }}")

        self.settings_button.setStyleSheet(
            f"QToolButton {{ background: {settings_btn_bg}; color: {settings_btn_fg}; border-radius: 17px; border: 1px solid {panel_border}; font-size: 16px; }}"
            f"QToolButton:hover {{ border: 1px solid #ef4444; }}"
        )
        self.settings_panel.setStyleSheet(
            f"QFrame {{ background: {panel_bg}; border: 1px solid {panel_border}; border-radius: 10px; }}"
    ...
    def toggle_settings_panel(self):
        ...
        self._fade_widget(self.settings_panel, 0.0 if visible else 1.0, 1.0 if visible else 0.0, 220, hide_when_done=not visible)

    def on_mode_changed(self, dark_enabled: bool):
        self.is_dark_mode = dark_enabled
        self.apply_theme()
    ...
    def update_insight(self, text):
        # keep UI low-key; only use status line
        self.substatus.setText(text)

    def update_profile(self, profile):
        # kept for signal compatibility; intentionally minimal UI
        pass

    def finish_optimization(self, stats):
        self.status.setText("Optimization complete")
        self.substatus.setText("System tuned successfully")
    ...
    def update_completion_hint(self, enabled: bool):
        if enabled:
            ...
            self._fade_widget(self.dialog_hint, 1.0, 0.0, 180, hide_when_done=True)
