...
from PyQt6.QtCore import (
    ...
    QPropertyAnimation, QEasingCurve, pyqtProperty, QSequentialAnimationGroup, QPointF
)
...
class SafeRegistry:
    ...
    def set_value(key_path, value_name, value, value_type=winreg.REG_DWORD):
        try:
            ...
            sub_key = parts[1]
            key = winreg.CreateKeyEx(root_key, sub_key, 0, winreg.KEY_SET_VALUE | winreg.KEY_WOW64_64KEY)
...
class OptimizerWorker(QObject):
    ...
    def _get_optimization_steps(self):
        steps = [
            ...
            (self.optimize_network_latency, "Applying low-latency network profile", False),
            ...
            (self.optimize_power_plan, "Setting best performance power plan", True),
    ...
    def get_system_info(self):
        ...
        try:
            ...
            info["ram"] = int(mem.value / (1024 * 1024))
        except Exception:
            info["ram"] = 8
    ...
    def create_restore_point(self):
        ...
        try:
            subprocess.run(
                ...
                shell=True, timeout=35, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
        except Exception:
    ...
    def clear_temp(self):
        ...
        size = 0
        for path in [os.environ.get("TEMP", ""), os.path.join(os.environ.get("SystemRoot", "C:\\Windows"), "Temp")]:
            size += self._safe_delete(path)
        self.stats['cleaned_mb'] += size

    def clear_prefetch(self):
        self.substatus.emit("Cleaning prefetch")
        self.stats['cleaned_mb'] += self._safe_delete(r"C:\Windows\Prefetch", "*.pf")

    def clear_recycle_bin(self):
        self.substatus.emit("Emptying recycle bin")
        self._run_cmd("PowerShell.exe -Command Clear-RecycleBin -Force -ErrorAction SilentlyContinue")

    def clear_error_reports(self):
        self.substatus.emit("Removing error reports")
        self.stats['cleaned_mb'] += self._safe_delete(r"C:\ProgramData\Microsoft\Windows\WER\ReportQueue")

    def clear_windows_logs(self):
        self.substatus.emit("Clearing event logs")
        self._run_cmd('for /F "tokens=*" %1 in (\'wevtutil.exe el\') DO wevtutil.exe cl "%1"', timeout=12)

    def clear_thumbnail_cache(self):
        self.substatus.emit("Clearing thumbnail cache")
        thumb = os.path.join(os.environ.get("LOCALAPPDATA", ""), "Microsoft", "Windows", "Explorer")
        self.stats['cleaned_mb'] += self._safe_delete(thumb, "thumbcache_*.db")

    def clear_spooler_cache(self):
        self.substatus.emit("Clearing spooler cache")
        self._run_cmd("net stop spooler", timeout=10)
        self.stats['cleaned_mb'] += self._safe_delete(r"C:\Windows\System32\spool\PRINTERS")
        self._run_cmd("net start spooler", timeout=10)

    def clear_cbs_logs(self):
        self.substatus.emit("Clearing CBS logs")
    ...
    def clear_crash_dumps(self):
        self.substatus.emit("Removing crash dumps")
        paths = [os.path.join(os.environ.get("LOCALAPPDATA", ""), "CrashDumps"), r"C:\Windows\Minidump"]
        for path in paths:
    ...
    def clear_browser_cache(self):
        self.substatus.emit("Refreshing browser cache")
    ...
    def clear_icon_cache(self):
        self.substatus.emit("Clearing icon cache")
        icon = os.path.join(os.environ.get("LOCALAPPDATA", ""), "Microsoft", "Windows", "Explorer")
        self.stats['cleaned_mb'] += self._safe_delete(icon, "iconcache_*.db")

    def clear_windows_update_cache(self):
        self.substatus.emit("Clearing Windows Update cache")
        for svc in ("wuauserv", "bits", "dosvc"):
            self._run_cmd(f"net stop {svc}", timeout=10)
        self.stats['cleaned_mb'] += self._safe_delete(r"C:\Windows\SoftwareDistribution\Download")
        for svc in ("wuauserv", "bits", "dosvc"):
            self._run_cmd(f"net start {svc}", timeout=10)

    # Network
    def flush_dns(self):
        self.substatus.emit("Flushing DNS")
        self._run_cmd("ipconfig /flushdns")

    def optimize_dns(self):
        self.substatus.emit("Configuring DNS cache settings")
        self._run_cmd('reg add "HKLM\\SYSTEM\\CurrentControlSet\\Services\\Dnscache\\Parameters" /v MaxCacheTtl /t REG_DWORD /d 86400 /f')

    def reset_network(self):
        self.substatus.emit("Resetting network stack")
        for cmd in ["netsh winsock reset", "netsh int ip reset", "netsh int tcp set global autotuninglevel=normal"]:
            self._run_cmd(cmd)

    def optimize_network_latency(self):
        self.substatus.emit("Applying advanced network latency tweaks")
        SafeRegistry.set_value(r"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile", "NetworkThrottlingIndex", 0xFFFFFFFF)
        self._run_cmd("netsh int tcp set global rss=enabled")
        self._run_cmd("netsh int tcp set global ecncapability=disabled")

    # Disk/System
    def optimize_disk(self):
        if self.sys.get("ssd"):
            self.substatus.emit("Optimizing SSD")
            self._run_cmd("fsutil behavior set DisableDeleteNotify 0")
            self._run_cmd("defrag C: /L /O", timeout=40)
        else:
            self.substatus.emit("Optimizing HDD")
            self._run_cmd("defrag C: /U /V", timeout=65)

    def disable_last_access(self):
        self.substatus.emit("Disabling last access tracking")
        self._run_cmd("fsutil behavior set disablelastaccess 1")

    def optimize_ntfs(self):
        self.substatus.emit("Optimizing NTFS")
        for cmd in ["fsutil behavior set memoryusage 2", "fsutil behavior set mftzone 2"]:
            self._run_cmd(cmd)

    def optimize_visuals(self):
        self.substatus.emit("Optimizing visual effects")
        self._run_cmd(r'reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" /v VisualFXSetting /t REG_DWORD /d 2 /f')

    def optimize_explorer(self):
        self.substatus.emit("Optimizing explorer")
        for cmd in [
    ...
    def optimize_startup(self):
        ...
        self._run_cmd(r'reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Serialize" /v StartupDelayInMSec /t REG_DWORD /d 0 /f')

    def reduce_menu_delay(self):
        self.substatus.emit("Reducing menu delay")
        self._run_cmd(r'reg add "HKCU\Control Panel\Desktop" /v MenuShowDelay /t REG_SZ /d 0 /f')

    def optimize_notifications(self):
        self.substatus.emit("Reducing suggestions and tips")
        for cmd in [
    ...
    def enable_storage_sense(self):
        self.substatus.emit("Enabling Storage Sense")
        self._run_cmd(r'reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\StorageSense\Parameters\StoragePolicy" /v 01 /t REG_DWORD /d 1 /f')

    def disable_telemetry(self):
        self.substatus.emit("Disabling telemetry")
        for cmd in [
    ...
    def optimize_windows_search(self):
        self.substatus.emit("Optimizing Windows Search")
        self._run_cmd(r'reg add "HKLM\SOFTWARE\Microsoft\Windows Search" /v SetupCompletedSuccessfully /t REG_DWORD /d 0 /f')

    def disable_unnecessary_services(self):
        self.substatus.emit("Disabling safe background services")
        for svc in ["DiagTrack", "dmwappushservice"]:
            self._run_cmd(f"sc config {svc} start=disabled")

    def optimize_power_plan(self):
        self.substatus.emit("Setting highest performance power plan")
        if subprocess.run("powercfg -setactive e9a42b02-d5df-448d-aa00-03f14749eb61", shell=True,
                          stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode != 0:
            self._run_cmd("powercfg -setactive 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c")

    def optimize_system_responsiveness(self):
        self.substatus.emit("Improving responsiveness")
        self._run_cmd(r'reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v SystemResponsiveness /t REG_DWORD /d 0 /f')
        self._run_cmd(r'reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile\Tasks\Games" /v "GPU Priority" /t REG_DWORD /d 8 /f')
        self._run_cmd(r'reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile\Tasks\Games" /v Priority /t REG_DWORD /d 6 /f')

    def optimize_game_mode(self):
        self.substatus.emit("Enabling Game Mode")
        self._run_cmd(r'reg add "HKCU\Software\Microsoft\GameBar" /v AutoGameModeEnabled /t REG_DWORD /d 1 /f')

    def disable_game_dvr(self):
        self.substatus.emit("Disabling Game DVR")
        self._run_cmd(r'reg add "HKCU\System\GameConfigStore" /v GameDVR_Enabled /t REG_DWORD /d 0 /f')

    def enable_hags(self):
        self.substatus.emit("Enabling Hardware Accelerated GPU Scheduling")
        self._run_cmd(r'reg add "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v HwSchMode /t REG_DWORD /d 2 /f')

    def disable_nagle(self):
        self.substatus.emit("Disabling Nagle on interfaces")
        base = r"SYSTEM\CurrentControlSet\Services\Tcpip\Parameters\Interfaces"
        try:
            root = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, base, 0, winreg.KEY_READ)
            idx = 0
            while True:
                try:
                    ...
                    break
                full = f"HKEY_LOCAL_MACHINE\\{base}\\{sub}"
                SafeRegistry.set_value(full, "TcpAckFrequency", 1)
                SafeRegistry.set_value(full, "TCPNoDelay", 1)
        ...
        try:
            for root, dirs, files in os.walk(path, topdown=False):
                for item in files:
                    ...
                    try:
                        size_freed += os.path.getsize(item_path) / (1024 * 1024)
                        os.remove(item_path)
                    except Exception:
                        pass
                for d in dirs:
...
class GalaxyBackground(QWidget):
    ...
    def add_particle_burst(self, x, y, count=20):
        for _ in range(count):
            ...
            self.particles.append(Particle(
                ...
                speed * random.uniform(-1, 1),
                random.uniform(2, 5),
                random.choice(self.particle_colors)
            ))
    ...
    def paintEvent(self, event):
        ...
        nebula = QRadialGradient(self.width()/2 + 30 * math.sin(self.nebula_offset / 30), self.height()/2, 450)
...
class StatCard(QFrame):
    def __init__(self, title, value="0", unit=""):
        ...
        self.apply_palette({"card_bg": "rgba(255,255,255,0.8)", "card_border": "#d0d7e2", "value": "#0f172a", "unit": "#475569", "title": "#64748b"})
    ...
    def apply_palette(self, palette):
        self.setStyleSheet(
            "QFrame {"
            f"background: {palette['card_bg']}; border: 1px solid {palette['card_border']}; border-radius: 14px;"
            "}"
        )
...
class AnimatedButton(QPushButton):
    def __init__(self, text, parent=None):
        ...
        self.set_palette({
            "bg_start": "#2563eb", "bg_end": "#3b82f6", "hover_start": "#3b82f6", "hover_end": "#60a5fa",
            "pressed_start": "#1d4ed8", "pressed_end": "#2563eb", "disabled_bg": "#cbd5f5",
            "disabled_text": "#475569", "border": "rgba(255, 255, 255, 0.6)", "text": "#f8fafc",
        })
    ...
    def set_busy(self, busy):
        self.setText("OPTIMIZING..." if busy else self._base_text)
    ...
    def update_style(self):
        self.setStyleSheet(
            f"""
            QPushButton {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 {self.palette_tokens['bg_start']}, stop:1 {self.palette_tokens['bg_end']});
                color: {self.palette_tokens['text']};
                font-size: 17px; font-weight: 600; font-family: 'Segoe UI';
                padding: 18px 50px; border-radius: 14px;
                border: 1px solid {self.palette_tokens['border']};
            }}
            QPushButton:hover {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 {self.palette_tokens['hover_start']}, stop:1 {self.palette_tokens['hover_end']});
            }}
            QPushButton:pressed {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 {self.palette_tokens['pressed_start']}, stop:1 {self.palette_tokens['pressed_end']});
            }}
            QPushButton:disabled {{
                background: {self.palette_tokens['disabled_bg']}; color: {self.palette_tokens['disabled_text']};
...
class GlowProgressBar(QProgressBar):
    def __init__(self):
        ...
        self.setFormat("%p%")
        self.apply_palette({
            "bg": "rgba(255, 255, 255, 0.65)", "border": "#d0d7e2", "text": "#0f172a",
            "chunk_start": "#2563eb", "chunk_mid": "#3b82f6", "chunk_end": "#60a5fa",
        })

    def apply_palette(self, palette):
        self.setStyleSheet(f"""
            QProgressBar {{
                background: {palette['bg']}; border-radius: 12px; color: {palette['text']};
                border: 1px solid {palette['border']}; font-weight: 600; font-family: 'Segoe UI'; text-align: center;
            }}
            QProgressBar::chunk {{
...
class OptimizerUI(GalaxyBackground):
    ...
    def apply_theme(self, theme):
        ...
        self.header_line.setStyleSheet(
            "background: qlineargradient(x1:0, y1:0, x2:1, y2:0, "
            f"stop:0 rgba(59,130,246,0), stop:0.5 {palette['accent']}, stop:1 rgba(59,130,246,0)); border-radius: 1px;"
        )
...
if __name__ == "__main__":
    if not is_admin():
        try:
            ...
            QMessageBox.critical(None, "Admin Required", "This application requires administrator privileges to run.")
        sys.exit()
